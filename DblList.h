...
T DblList<T>::removeBack(){ // constant
    if(isEmpty()){
        cout << "The list is empty so there is nothing to remove!" << endl;
    }
...
T DblList<T>::get(int pos){
    if(isEmpty()){
        cout << "The list is empty so there is nothing to get!" << endl;
    } else if(pos < 0 || pos >= m_size){
        cout << "That position is out of range!" << endl;
    } else {
        ListNode<T>* current = m_front;
        int cPos = 0;
        while (cPos != pos){
